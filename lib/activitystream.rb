module Activitystream
	def add_activity_stream(modulename, moduleid, actionname)
		...
		if modulename.to_s == 'COURSE'
			...
			if actionname.to_s == "published"
				...
				message = "A new course named '<a href='"+learner_path(coursedetail)+"'>"+coursedetail.title.capitalize+"</a>' has been published under "+coursedetail.subjects.first.name.capitalize+"."
			end
			
			if actionname.to_s == "subscribe"
				notification_for = "Teacher"
				message = "A new user has subsribed for your course '<a href='"+learner_path(coursedetail)+"'>"+coursedetail.title.capitalize+"</a>."
			end
		end
	...
	def get_activity_stream()
		user_notifications = Notification.find(:all, :conditions => ["notification_for = ? and module_id IN (?)", current_user.type, Course.includes(:subjects).where("subjects.id IN (?)", current_user.subjects.map( &:id)).map( &:id)])
	end
